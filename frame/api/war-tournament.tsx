...
import {
  checkTournamentInvitation,
  publicClient,
  tournamentCardContract,
  tournamentWarContract,
  tournamentWarPoolContract,
...
export const warTournamentApp = new Frog<{ State: WarState }>({
  initialState: {
    quantities: [],
    address: '',
    pfp_url: '',
...
warTournamentApp.frame('/make-duel/:game_mode', async (c) => {
  if (IS_MAINTENANCE)
  ...
  const c_address = c.previousState.c_address;
  if (!c_address) return c.error({ message: 'Invalid request' });
  ...
  const [hasNFT, c_hasNFT, quantities] = await Promise.all([
    checkTournamentInvitation(address),
    checkTournamentInvitation(c_address),
  ...
  if (!hasNFT || !c_hasNFT) {
    return c.res({
      ...
      imageAspectRatio: '1:1',
      intents: [<Button action={`${BASE_URL}/top`}>Back</Button>],
    });
  }
...
warTournamentApp.frame('/preview', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
  ...
  if (numOfCards && numOfCards > 0) {
    ...
    const isValidCardCount = [5].includes(convertedArray.length);
    if (!isValidCardCount) {
      return c.error({
  ...
  if (errorMessages.length > 0) {
    ...
    return c.res({
      title,
      image: `/war-tournament/image/error/${params}`,
      imageAspectRatio: '1:1',
  ...
  return c.res({
    title,
    image: '/war-tournament/image/preview/' + params,
    imageAspectRatio: '1:1',
    action: '/find',
    intents: [
...
warTournamentApp.transaction('/duel-letter', async (c) => {
  ...
  const estimatedGas = await tournamentWarContract.estimateGas.makeGame(args, {
    account: address,
  });

  return c.contract({
    chainId: 'eip155:666666666',
    to: WAR_TOURNAMENT_CONTRACT_ADDRESS,
    abi: WAR_ABI,
...
warTournamentApp.frame('/find', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
  ...
  const game = await tournamentWarContract.read.games([gameId]);
  const createdAt = game[6];
  const numOfCards = card.length;
  const sumOfCards = Number(
    await tournamentWarContract.read.sumOfCards([gameId]),
  );
  await setGameInfo(
    gameId,
    address,
  ...
  const shareLink = `${shareUrlBase}${
    c_userName
      ...
      : shareText
  }${embedParam}${BASE_URL}/war-tournament/challenge/${gameId}`;

  return c.res({
    title,
    image: '/war-tournament/image/find/' + params,
    browserLocation: '/war-tournament/image/find/' + params,
...
const generateErrorImage = async (
  ...
  const svgOverlay = await sharp('./public/images/war-tournament/error.png')
    .resize(1000, 1000)
    .png()
    .toBuffer();
...
warTournamentApp.frame('/error/address', (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
...
warTournamentApp.frame('/challenge/:gameId', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
...
const challengeFrame = async (
  ...
  if (!gameInfo) {
    try {
      ...
      const numOfCards = Number(
        await tournamentWarContract.read.numOfCards([gameId]),
      );
      const sumOfCards = Number(
        await tournamentWarContract.read.sumOfCards([gameId]),
      );
      ...
      const createdAt = game[6];
      const gameDeposits = await tournamentWarPoolContract.read.gameDeposits([
        gameId,
      ]);
      ...
      if (makerCard !== BigInt(0)) {
        ...
        const makerCards: number[] = await Promise.all(
          Array.from({ length: numOfCards }, async (_, i) => {
            return Number(
              await tournamentWarContract.read.playerCards([
                makerCard,
                BigInt(i),
              ]),
            );
  ...
  return c.res({
    title,
    image:
      '/war-tournament/image/challenge/' +
      encodeURIComponent(
        JSON.stringify({
          userName,
          pfp_url,
...
warTournamentApp.frame('/choose', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
...
warTournamentApp.frame('/choose/:params', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
  ...
  const gameStatus = await tournamentWarContract.read.gameStatus([gameId]);
  ...
  // direct match
  const requestedChallengers =
    await tournamentWarContract.read.requestedChallengers([gameId]);
...
warTournamentApp.frame('/duel', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
...
warTournamentApp.transaction('/challengeGame', async (c) => {
  ...
  try {
    ...
    const estimatedGas = await tournamentWarContract.estimateGas.challengeGame(
      args,
      {
        account: c_address,
      },
...
warTournamentApp.frame('/loading', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
...
warTournamentApp.frame('/result/:gameId', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
  ...
  if (recentCard && recentCard.length > 0) {
    ...
    [gameInfo, contractResult] = await Promise.all([
      ...
      tournamentWarContract.read.games([gameId]),
    ]);
  } else {
    gameInfo = await getGameInfoByGameId(gameId);
  }
  ...
  if (!winner || winner === zeroAddress) {
    if (!contractResult) {
      contractResult = await tournamentWarContract.read.games([gameId]);
    }
    ...
    for (let i = 0; i < numOfCards; i++) {
      const [makerCard, challengerCard] = await Promise.all([
        tournamentWarContract.read.playerCards([
          makerCardIdentifier,
          BigInt(i),
        ]),
        tournamentWarContract.read.playerCards([
          challengerCardIdentifier,
          BigInt(i),
        ]),
      ]);
  ...
  if (winner?.toLowerCase() === c_address?.toLowerCase()) {
    shareLink = `${shareUrlBase}Victory is mine! I beat @${userName} 🏆%0A${embedParam}${BASE_URL}/war-tournament/result/${gameId}`;
  } else if (winner?.toLowerCase() === zeroAddress) {
    shareLink = `${shareUrlBase}I drew with @${userName} 🤝%0A${embedParam}${BASE_URL}/war-tournament/result/${gameId}`;
  } else {
    shareLink = `${shareUrlBase}I lost to @${userName} 😭%0A${embedParam}${BASE_URL}/war-tournament/result/${gameId}`;
  }
  ...
  return c.res({
    title,
    image: `/war-tournament/image/result/${encodedResultParams}`,
    imageAspectRatio: '1:1',
    browserLocation: `/war-tournament/image/result/${encodedResultParams}`,
    intents,
  });
});

warTournamentApp.frame('/make-direct-duel/:castHash', async (c) => {
  if (IS_MAINTENANCE)
    return c.error({ message: 'Under maintenance, please try again later.' });
...
const generateLoadingImage = async (
  ...
  const overlayImage = await sharp(
    './public/images/war-tournament/loading_overlay.png',
  )
    .resize(1000, 1000)
    .toBuffer();
...
warTournamentApp.hono.get('/image/error/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
...
warTournamentApp.hono.get('/image/challenge/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
  const {
    userName,
    pfp_url,
  ...
  const status = await tournamentWarContract.read.gameStatus([gameId]);
...
warTournamentApp.hono.get('/image/duel/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
  const {
    userName,
    pfp_url,
...
warTournamentApp.hono.get('/image/loading/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
  const {
    userName,
    pfp_url,
...
const getQuantities = async (address: string, c: any) => {
  ...
  const data = await tournamentCardContract.read.balanceOfBatch([
    addressList,
    allCardIds,
  ]);
...
const getSignature = async (c: any): Promise<string> => {
  try {
    ...
    const response = await fetch(`${BACKEND_URL!}/war/sign-tournament`, {
      method: 'POST',
      headers: {
...
warTournamentApp.hono.get('/image/score/:quantities', async (c) => {
...
warTournamentApp.hono.get('/image/preview/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
...
warTournamentApp.hono.get('/image/find/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
  const {
    userName,
    pfp_url,
...
warTournamentApp.hono.get('/image/expired/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
  const {
    userName,
    pfp_url,
...
warTournamentApp.hono.get('/image/result/:params', async (c) => {
  const params = JSON.parse(decodeURIComponent(c.req.param('params')));
  const {
    userName,
    pfp_url,
...
const generateOwnCard = async (
  ...
  const baseImage = sharp(
    `./public/images/war-tournament/pick_${numOfCards || ''}card.png`,
  ).resize(1000, 1000);
...
const generateChallengeImage = async (
  ...
  const canvas = sharp('./public/images/war-tournament/' + imageName).resize(
    1000,
    1000,
  );
...
const generateMultiChallengeImage = async (
  ...
  const canvas = sharp(
    './public/images/war-tournament/multi/' + imageName,
  ).resize(1000, 1000);
...
const generateMultiShareImage = async (
  ...
  const composites = [
    ...
    { input: sumOfCardsImage, left: 589, top: 168 },
  ];
...
const generatePreviewImage = async ({
  ...
  const [userComponent, cardImage, wagerImage] = await Promise.all([
    ...
    sharp(`./public/images/war-tournament/card/${card}.png`)
      .resize(185)
      .png()
      .toBuffer(),
    isBet
...
const generateMultiPreviewImage = async ({
  ...
  const cardImagePromises = card
    .slice(0, handSize)
    .map((cardValue) =>
      sharp(`./public/images/war-tournament/multi/card/${cardValue}.png`)
        .resize(cardWidth)
        .png()
        .toBuffer(),
    );
...
const generatePlayedSimpleImage = async () => {
  const canvas = sharp('./public/images/war-tournament/played.png').resize(
    1000,
    1000,
  );
...
const generateExpiredSimpleImage = async () => {
  const canvas = sharp('./public/images/war-tournament/expired.png').resize(
    1000,
    1000,
  );
...
const generateExpiredImage = async ({
  ...
  const overlayImage = await sharp(
    './public/images/war-tournament/expired_overlay.png',
  )
    .resize(1000, 1000)
    .toBuffer();
...
const generateDuelImage = async (
  ...
  const baseImage = sharp('./public/images/war-tournament/' + imageName).resize(
    1000,
    1000,
  );
...
const generateResultBgImage = async (
  ...
  //       .png()
  //       .toBuffer(),
  //     sharp(`./public/images/war-tournament/card/${c_card}.png`)
  //       .resize(185)
  //       .png()
  //       .toBuffer(),
  //     generateUserComponent(userName, pfp_url),
...
const generateResultImage = async (
  ...
  const overlayImage = await sharp(
    './public/images/war-tournament/' + overlayImageName,
  )
    .resize(1000, 1000)
    .toBuffer();
      ...
      card.map((cardValue, index) => {
        const cardPath =
          results[index] === Result.Win
            ? `./public/images/war-tournament/multi/card/${cardValue}.png`
            : `./public/images/war-tournament/multi/card/dark/${cardValue}.png`;
...
const generateJokerKillingImage = async (
  ...
  const container = sharp(
    `./public/images/war-tournament/jokerkilling/jokerkilling_${imageNo}_${jokerKilling.toLowerCase()}.png`,
  ).resize(1000, 1000);
